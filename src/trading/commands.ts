 ...
 * Paper simulator edition — /portfolio /watchlist /status /run_now /exit_now.
 * Uses simulated broker (Yahoo Finance prices, no external keys).
 */
  ...
  bot.command("exit_now", async (ctx: Context) => {
    ...
    try {
      ...
      if (result.submitted === 0 && result.skipped === 0 && result.failed === 0) {
        lines.push("No entries for today — nothing to exit. Run /run_now first (or wait for the 15:45 window).");
      }
      await sendLongMessage(ctx as unknown as Parameters<typeof sendLongMessage>[0], lines.join("\n"));
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
